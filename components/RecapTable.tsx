const ROWS = [
  ...
  { name: "Rotation", matrix: "R(θ)", det: "1", eigen: "cosθ ± i·sinθ" },
        ...
        <tbody>
          {ROWS.map((row) => (
            <tr key={row.name} className="border-b border-[rgba(0,0,0,0.06)] last:border-b-0">
              ...
              <td className="px-4 py-3 font-mono text-[13px] text-foreground">{row.matrix}</td>
      ...
      <div className="divide-y divide-[rgba(0,0,0,0.06)] sm:hidden">
        {ROWS.map((row) => (
          <div key={row.name} className="space-y-1.5 px-4 py-3">
            ...
            <div className="flex justify-between font-mono text-[13px] text-foreground-soft">
              <span>matrix</span>
              <span className="text-foreground">{row.matrix}</span>
            </div>
